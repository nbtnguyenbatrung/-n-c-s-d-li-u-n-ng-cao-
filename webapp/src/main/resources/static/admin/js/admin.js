$(document).ready(function(){
    ...
    $('body').on('click','.editsanphamsize',function(){
         ...
         $.get(href,function(sanphamsize,status){
            ...
            $('.myform #soluong').val(sanphamsize.soluong);
         });
    });

    $('#addspsize').click(function(event){
       if ( $('#maspadd').val() == false || $('#masizeadd').val() == false || $('#soluongadd').val() == false){
            alert(" Không được để chống các trường có ký hiệu * ! ");
       }
       else {
          $.ajax({
    ...
    $('#editsanphamsize').click(function(event){
        if ( $('#masp').val() == false || $('#masize').val() == false || $('#soluong').val() == false ){
            alert(" Không được để chống các trường có ký hiệu * ! ");
        }
        else{
           $.ajax({
             url: "http://localhost:9999/updatespsize",
             type: "GET",
             data: {
                 maSP : $('#masp').val(),
                 maSize : $('#masize').val(),
